...
module.exports.requestOptsToCurl = function(requestOpts){
	return "curl -X "+requestOpts.method+" '"+requestOpts.protocol+'//'+requestOpts.hostname+(requestOpts.port?':'+requestOpts.port:'')+requestOpts.path+"' "+Object.keys(requestOpts.headers).reduce(function(prev,key){return prev+" -H '"+key+": "+requestOpts.headers[key]+"'"},'')+(requestOpts.method!='GET'?" --data '"+requestOpts.data+"'":"")
}
