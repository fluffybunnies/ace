				...
				if (words[i]) {
					...
					if (isName) {
						...
						if ((fi = words[i].indexOf('-')) != -1) {
							console.log('WEF',fi,words[i].substring(0,i-1) ,words[i][fi-1].toUpperCase() ,words[i].substr(fi+1));
							words[i] = words[i].substring(0,i-1) + words[i][fi-1].toUpperCase() + words[i].substr(fi+1);
						}
					}
				}
			}
